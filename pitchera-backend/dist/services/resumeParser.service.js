...
function extractDataFromText(text) {
    ...
    // LinkedIn
    const linkedinMatch = text.match(/(?:linkedin\.com\/in\/|linkedin\.com\/profile\/)([\w-]+)/i);
    if (linkedinMatch)
    ...
    if (educationSection) {
        ...
        for (const line of eduLines) {
            ...
            if (line.match(/\b(bachelor|master|b\.?tech|m\.?tech|b\.?e|m\.?e|b\.?sc|m\.?sc|diploma|10th|12th|phd)\b/i)) {
            ...
            else if (line.match(/\b(university|college|institute|school|iit|nit|bits)\b/i)) {
    ...
    if (experienceSection) {
        ...
        for (const line of expLines) {
            const dateRange = line.match(/(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec|january|february|march|april|june|july|august|september|october|november|december)[\s,]+\d{4}/i);
            if (dateRange) {
            ...
            else if (line.match(/\b(engineer|developer|manager|analyst|designer|consultant|lead|senior|junior|intern|associate)\b/i)) {
                currentExp.designation = line.trim();
            }
            else if (line.match(/\b(pvt|ltd|inc|corp|technologies|solutions|systems|company)\b/i) ||
